  ...
  class Reactor : public ::grpc::experimental::ServerUnaryReactor {
   public:
    Reactor(CallbackTestServiceImpl* service,
      ...
      // the result. Call it asynchronously to see if we trigger any data races. Join it in OnDone (technically that could be blocking but shouldn't be for very long).
	...
	// Block on the "wait to continue" decision in a different thread since we can't tie up an EM thread with blocking events. We can join it in OnDone since it would definitely be done by then.
	rpc_wait_thread_ = std::thread([this]{service_->signaller_.ServerWaitToContinue(); StartRpc();});
      } else {
      StartRpc();
      }
    }
        ...
        alarm_.experimental().Set(
            gpr_time_add(gpr_now(GPR_CLOCK_MONOTONIC),
                         gpr_time_from_micros(
                             req_->param().server_sleep_us(), GPR_TIMESPAN)),
