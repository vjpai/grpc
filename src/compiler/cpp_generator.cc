...
void PrintHeaderClientMethodInterfaces(
  ...
  if (is_public) {
    if (method->NoStreaming()) {
      ...
      for (auto async_prefix : {"Async", "PrepareAsync"}) {
        (*vars)["AsyncPrefix"] = async_prefix;
        printer->Print(
            *vars,
            "std::unique_ptr< "
      ...
      for (auto async_prefix : {"Async", "PrepareAsync"}) {
        ...
        printer->Print(
            *vars,
            ...
            "$AsyncPrefix$$Method$Raw(context, response, cq, tag));\n");
        printer->Outdent();
        printer->Print("}\n");
      }
      ...
      for (auto async_prefix : {"Async", "PrepareAsync"}) {
        ...
        printer->Print(
            *vars,
            ...
            "$AsyncPrefix$$Method$Raw(context, request, cq, tag));\n");
        printer->Outdent();
        printer->Print("}\n");
      }
      ...
      for (auto async_prefix : {"Async", "PrepareAsync"}) {
        ...
        printer->Print(
            *vars,
            "return std::unique_ptr< "
            "::grpc::ClientAsyncReaderWriterInterface< $Request$, $Response$>>("
            "$AsyncPrefix$$Method$Raw(context, cq, tag));\n");
        printer->Outdent();
        printer->Print("}\n");
      }
    }
...
void PrintSourceClientMethod(grpc_generator::Printer *printer,
  ...
  } async_prefixes[] = {{"Async", "true"}, {"PrepareAsync", "false"}};
  if (method->NoStreaming()) {
    printer->Print(*vars,
    ...
    for (auto async_prefix : async_prefixes) {
      ...
      printer->Print(*vars,
                     ...
                     "channel_.get(), cq, "
                     "rpcmethod_$Method$_, "
                     "context, response, tag, $AsyncStart$);\n"
                     "}\n\n");
    }
  } else if (ServerOnlyStreaming(method)) {
    printer->Print(
    ...
    for (auto async_prefix : async_prefixes) {
      ...
      printer->Print(*vars,
                     ...
                     "channel_.get(), cq, "
                     "rpcmethod_$Method$_, "
                     "context, request, tag, $AsyncStart$);\n"
                     "}\n\n");
    }
  } else if (method->BidiStreaming()) {
    printer->Print(
...
void PrintMockClientMethods(grpc_generator::Printer *printer,
  ...
  if (method->NoStreaming()) {
    ...
    for (auto async_prefix : {"Async", "PrepareAsync"}) {
      (*vars)["AsyncPrefix"] = async_prefix;
      printer->Print(*vars,
                     "MOCK_METHOD4($AsyncPrefix$$Method$Raw, "
                     "::grpc::ClientAsyncWriterInterface< $Request$>*"
                     "(::grpc::ClientContext* context, $Response$* response, "
                     "::grpc::CompletionQueue* cq, void* tag));\n");
    }
